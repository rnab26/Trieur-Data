...
def post_record_tag(org_id: str, record_id: str, body: RecordTagCreate, ctx: AuthCtx = Depends(require_org_access)):
    tag = body.tag.strip()
    if not tag:
...
def delete_record_tag(org_id: str, record_id: str, tag: str, ctx: AuthCtx = Depends(require_org_access)):

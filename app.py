...
if "auto_assign_done" not in st.session_state:
...
def auto_assign_all_sheets(all_sheets, master_columns):
    new_mappings = {}
...
with tab2:
    ...
    files = st.file_uploader(
        "Deposez un ou plusieurs fichiers Excel",
        type=["xlsx", "xls"],
    ...
    if all_sheets:
        ...
        st.markdown("---")
        st.subheader("Assignation automatique globale")

        col_a, col_b, col_c = st.columns([2, 1, 3])

        with col_a:
            if st.button("🚀 Auto assign all sheets", type="primary"):
                st.session_state.sheet_mappings = auto_assign_all_sheets(
                    all_sheets,
                    st.session_state.master_columns
                )
                st.session_state.auto_assign_done = True
                st.success("✅ Assignation automatique terminée pour tous les onglets.")
                st.rerun()
        ...
        with col_b:
            if st.button("♻️ Reset mappings"):
                st.session_state.sheet_mappings = {}
                st.session_state.auto_assign_done = False
                st.success("Mappings réinitialisés.")
                st.rerun()

        with col_c:
            st.caption("L'assignation automatique est exécutée une seule fois pour tous les onglets avant l'édition manuelle.")

        st.markdown("---")
        st.subheader("Assignation manuelle par onglet")
        ...
        for sheet_key, sheet_df in all_sheets.items():
            ...
            mapping_options = [NON_ASSIGNE] + st.session_state.master_columns
            updated_mapping = {}

            st.write("**Sélectionnez les colonnes maîtres correspondantes :**")

            cols_display = st.columns(len(real_columns)) if len(real_columns) > 0 else []
            ...
            for idx, src_col in enumerate(real_columns):
                with cols_display[idx]:
                    ...
                    already_used = [
                        updated_mapping.get(c, "")
                        for c in real_columns
                        if c != src_col
                    ]

                    available_options = [NON_ASSIGNE] + [
            ...
            st.write("**Aperçu des 7 premières lignes :**")
            ...
            if st.button("✅ Construire la base de travail fusionnee", type="primary"):
                ...
                for sheet_key, sheet_df in all_sheets.items():
                    ...
                    if not assigned_cols:
                        st.warning(f"⚠️ {sheet_key}: aucune colonne assignée, onglet ignoré.")
                        continue
...
with tab3:
    ...
    if st.session_state.final_df is None:
        ...
        if filter_col == "CP":
            ...
            selected_vals = st.multiselect(f"Valeurs a conserver pour {filter_col}", options=unique_vals)
            if selected_vals:
        ...
        st.write(
            f"Resultat filtre : **{remaining_lines}** lignes | "
            f"**{remaining_duplicates}** doublons conserves | "
            f"**{total_lines}** lignes importees au total"
        )
